...
def showData():
    #读取数据
    lstm_data_csv = pd.read_csv('./mei_di_lstm_v1.csv', usecols=[0,1])
    #去除读入的数据中含有NaN的行。
    ...
    #读取数据
    gru_data_csv = pd.read_csv('./mei_di_gru_v1.csv', usecols=[1])
    #去除读入的数据中含有NaN的行。
    gru_data_csv = gru_data_csv.dropna()
    gru_data_set = gru_data_csv.values
    #对数据类型进行转换
    gru_data_set = gru_data_set.astype('float32')

    #读取数据
    bp_data_csv = pd.read_csv('./mei_di_bp_v1.csv', usecols=[1])
    #去除读入的数据中含有NaN的行。
    ...
    #读取数据
    svm_data_csv = pd.read_csv('./mei_di_svm.csv', usecols=[1])
    #去除读入的数据中含有NaN的行。
    ...
    gru_data_set = gru_data_set[-len(lstm_data_set):]
    ...
    for e in range(date_num):
        ...
        gru_current_data = gru_data_set[e*24:(e+1)*24]
        ...
        gru_current_mape = mean_absolute_percentage_error(temp_real, temp_gru_pred)
        ...
        gru_current_mse = mean_squared_error(temp_real, temp_gru_pred)
        ...
        gru_current_rmse = np.sqrt(gru_current_mse)
        bp_current_rmse = np.sqrt(bp_current_mse)
        svm_current_rmse = np.sqrt(svm_current_mse)
        print('e: {}, lstm_mape: {}, lstm_mse: {}, lstm_rmse: {}'.format(e-6 , lstm_current_mape, lstm_current_mse, lstm_current_rmse))
        print('e: {}, gru_mape: {}, gru_mse: {}, gru_rmse: {}'.format(e-6 , gru_current_mape, gru_current_mse, gru_current_rmse))
        print('e: {}, bp_mape: {}, bp_mse: {}, bp_rmse: {}'.format(e-6 , bp_current_mape, bp_current_mse, bp_current_rmse))
        print('e: {}, svm_mape: {}, svm_mse: {}, svm_rmse: {}'.format(e-6 , svm_current_mape, svm_current_mse, svm_current_rmse))
        gru+=gru_current_mape
    ...
    print("gru_total mape:{}, mse:{},rmse:{}",mean_absolute_percentage_error(real_total_data, gru_total_data), mean_squared_error(real_total_data, gru_total_data), np.sqrt(mean_squared_error(real_total_data, gru_total_data)))
    ...
    #7月31-8月6号
    # lstm_min_data = lstm_min_data[-21*24:-14*24]
    # gru_min_data = gru_min_data[-21*24:-14*24]
    # bp_min_data = bp_min_data[-21*24:-14*24]
    # svm_min_data = svm_min_data[-21*24:-14*24]

    #8月15
    # lstm_min_data = lstm_min_data[-6*24:-5*24]
    # gru_min_data = gru_min_data[-6*24:-5*24]
    # bp_min_data = bp_min_data[-6*24:-5*24]
    # svm_min_data = svm_min_data[-6*24:-5*24]

    real = lstm_min_data[+0:, 0]  #取电量值
    lstm_pred = lstm_min_data[+0:, 1]  #取电量值
    gru_pred = gru_min_data[+0:, 0]  #取电量值
    ...
    plt.plot(gru_pred,  label='gru', linewidth=0.5)
