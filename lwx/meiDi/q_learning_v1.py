...
ACTIONS = np.round(np.linspace(0.3, 1.2, num=12), decimals=3)     # 动作，离散化   零售价
...
ELASTIC_COEFFICIENT = np.array([-0.3, -0.5, -0.7])  #谷、平、峰需求响应的弹性系数
...
CONVERGENCE_THRESHOLD = 0.0001  #判断是否收敛
...
def choose_action(S, q_table, dataset,epsilon):
    if np.random.uniform() > epsilon:
        ...
        #     # 检查零售价是否满足条件
        #     # if ACTIONS[i] >= PREDICT_ACTIONS[TIME_DICT[S]] * 1.01:
        #     if 1==1:
        #         valid_actions.append(ACTIONS[i])

        if len(ACTIONS) > 0:
            for j in range(len(ACTIONS)):
                S_, R = get_env_feedback(S, ACTIONS[j], dataset, 0)
                # 更新最大奖励和对应的动作
                if max_reward < R:
                    max_reward = R
...
def test():
    ...
    for episode in range(MAX_EPISODES):
        ...
        while not is_terminated:
            ...
            #获取下一个状态，计算奖励
            S_, R = get_env_feedback(S, A, dataset, 1)
            #估计值
            ...
            q_table.loc[S, A] += LEARNING_RATE * (q_target - q_predict)
            S = S_
    ...
    # 获取 Q 值不为零的最大值对应的动作
    ACTIONS_LIST = q_table.loc[:, (q_table != 0).any(axis=0)].idxmax(axis=1)
    print("-------------------最合适零售价--------------------------")
    #用户总成本-优化前
