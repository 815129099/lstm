...
def run():
    #读取数据
    data_csv = pd.read_csv('./2017.5.csv', usecols=[0,1,2])
    ...
    #保存模型
    torch.save(net.state_dict(), "D:\lunwengit\LTSF-Linear-main\lwx\lstm.pth")
...
def run1():
    ...
    # 模型加载
    net = lstm_reg(3, 4).to(device)
    net.load_state_dict(torch.load("D:\lunwengit\LTSF-Linear-main\lwx\lstm.pth"))
    # model.eval()
