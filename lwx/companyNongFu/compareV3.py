...
def showData():
    ...
    #读取数据
    bp_data_csv = pd.read_csv('./new_data_nong_lstm_v2.csv', usecols=[0])
    #去除读入的数据中含有NaN的行。
    ...
    for e in range(date_num):
        ...
        print('e: {}, mrmr_lstm_mape: {}, mrmr_lstm_mse: {}, mrmr_lstm_rmse: {}'.format(e , lstm_current_mape, lstm_current_mse, lstm_current_rmse))
        print('e: {}, lstm_mape: {}, lstm_mse: {}, lstm_rmse: {}'.format(e , bp_current_mape, bp_current_mse, bp_current_rmse))
    ...
    print("mrmr_lstm_total mape:{}, mse:{},rmse:{}",mean_absolute_percentage_error(real_total_data, lstm_total_data), mean_squared_error(real_total_data, lstm_total_data), np.sqrt(mean_squared_error(real_total_data, lstm_total_data)))
    print("lstm_total mape:{}, mse:{},rmse:{}",mean_absolute_percentage_error(real_total_data, bp_total_data), mean_squared_error(real_total_data, bp_total_data), np.sqrt(mean_squared_error(real_total_data, bp_total_data)))
    ...
    plt.plot(bp_pred, 'g', label='LSTM', linewidth=0.5)
    plt.legend(loc='best')
    plt.xlabel("时间点/h")
    plt.ylabel("负荷/kW")
    plt.show()
