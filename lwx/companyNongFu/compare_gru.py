...
def showData():
    ...
    #读取数据
    gru_data_csv = pd.read_csv('./new_data_nong_gru_v1.csv', usecols=[1])
    #去除读入的数据中含有NaN的行。
    ...
    for e in range(date_num):
        ...
        temp_svm_pred = svm_current_data[+0:, 0]  # 取svm预测电量值
        ...
        print('e: {}, gru_mape: {}, gru_mse: {}, gru_rmse: {}'.format(e , gru_current_mape, gru_current_mse, gru_current_rmse))
    ...
    # #8月5号
    # lstm_min_data = lstm_min_data[-16*24:-15*24]
    # bp_min_data = bp_min_data[-16*24:-15*24]
    # svm_min_data = svm_min_data[-16*24:-15*24]
    # gru_min_data = gru_min_data[-16*24:-15*24]
    # #8月14号-8月20
    # lstm_min_data = lstm_min_data[-7*24:]
    # bp_min_data = bp_min_data[-7*24:]
    # svm_min_data = svm_min_data[-7*24:]
    # gru_min_data = gru_min_data[-7*24:]
