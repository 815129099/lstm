...
def run():
    ...
    train_data = real_set[:train_size]
    ...
    test_data = test_data.reshape(len(test_data), 1, 5)
    ...
    pred_test = inverse_transform_col(min_max_scaler, pred_test, n_col=4)  # 对预测值反归一化

    # data_csv = data_csv.drop(data_csv.index[0:train_size-1])
    #追加
    real_data_csv = pd.DataFrame(data=real_set[train_size:],columns=['month','date','hour','peak_flat_valley','real'])
    pred_data_csv = pd.DataFrame(data=pred_test,columns=['pred'])
    dataframe = real_data_csv.join(pred_data_csv)
    dataframe.to_csv('./new_data_nong_lstm.csv',index=False,mode='w',sep=',')

    # pred_test = pred_test[-24:]
    # real_set = list(map(lambda x: x * scalar, real_set))
    real_set = real_set[train_size:]
    real_set = real_set[+0:, 4]  #取电量值
...
def showData():
    #读取数据
    lstm_data_csv = pd.read_csv('./new_data_nong_lstm.csv', usecols=[4,5])
    #去除读入的数据中含有NaN的行。
