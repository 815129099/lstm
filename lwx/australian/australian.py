...
def run():
    #读取数据
    data_csv = pd.read_csv('./Australian_electricity_data.csv', usecols=[1,2,3,4,5,7])
    ...
    #序列长度、序列个数、特征数
    train_X = train_X.reshape(len(train_X),1, 6)
    ...
    test_X = test_X.reshape(len(test_X), 1, 6)
    ...
    #保存模型
    torch.save(net.state_dict(),"D:\lunwengit\LTSF-Linear-main\lwx\laustralian.pth")
    # plt.plot(data_csv)
    # plt.show()
...
def load():
    # 读取数据
    data_csv = pd.read_csv('./Australian_electricity_data_1.csv', usecols=[0, 1, 2, 3, 4, 5])
    ...
    # net = lstm_reg(6, 4).to(device)
    net = lstm_reg(6, 4).to(device)
    net.load_state_dict(torch.load("D:\lunwengit\LTSF-Linear-main\lwx\laustralian.pth"))
    ...
    dataframe.to_csv('./Australian_electricity_data_2.csv',index=False,mode='w',sep=',')
